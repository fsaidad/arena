      ...
      <section className="control-workspace" id="overview">
        ...
        <section className="participants-panel" id="participants">
          ...
          <div className="participant-table" role="table" aria-label="Tournament participants">
            ...
            {participants.map((item, index) => <div className="table-row" role="row" key={item.id}><span role="cell">{String(index + 1).padStart(2, "0")}</span><span role="cell"><i className="participant-avatar">{item.name.slice(0, 2).toUpperCase()}</i><b>{item.name}</b></span><span role="cell">{item.region}</span><span role="cell"><em className={item.status === "Checked in" ? "ok" : "pending"}>{item.status}</em></span><span role="cell"><button aria-label={`Participant ${index + 1} actions`}><MoreHorizontal size={17} /></button></span></div>)}
          </div>
        </section>
