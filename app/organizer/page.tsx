...
import { Activity, ArrowLeft, CalendarDays, Check, ChevronRight, CircleGauge, Clock3, Plus, Radio, RotateCcw, ShieldCheck, Users } from "lucide-react";
...
export default function OrganizerPage() {
  ...
  const { homeScore, score, setScore, matchStatus, version, mode, saving, notice, setNotice, publishResult } = useMatchControl(role);
    ...
    <main className="control-shell">
      ...
      <section className="control-workspace" id="overview">
        <header className="control-header">
          ...
          <div className="operator-tools"><div className="role-control"><label htmlFor="role">Demo role</label><select id="role" value={role} onChange={(event) => setRole(event.target.value as DemoRole)}><option value="viewer">Viewer</option><option value="operator">Operator</option><option value="admin">Admin</option></select></div></div>
        </header>
        ...
        <section className="metric-strip" aria-label="Tournament summary">
          ...
          <div><span>Matches today</span><strong>18</strong><p>{matchStatus === "completed" ? "7 completed · 0 live" : "6 completed · 1 live"}</p></div>
        ...
        <div className="control-grid">
          <section className="match-control-panel" id="matches">
            <div className="control-section-heading"><div><span className={matchStatus === "live" ? "live-chip" : "status-chip"}><i /> {matchStatus === "completed" ? "Completed" : matchStatus === "live" ? "Live" : "Scheduled"}</span><h2>Upper final</h2><p>Best of 3 · Map 2 — Ancient</p></div></div>
            <div className="control-score">
              <div><span className="control-monogram">BK</span><label htmlFor="homeScore">Black Kite</label><input id="homeScore" type="number" value={homeScore} readOnly /></div>
            ...
            <div className="publish-bar"><div><span>Publishing updates</span><strong>Score · live clients · audit trail</strong></div><button onClick={() => void publishResult(score)} disabled={role === "viewer" || mode === "connecting" || saving}>{saving ? "Publishing…" : matchStatus === "completed" ? "Update result" : "Publish result"} <ChevronRight size={17} /></button></div>
          ...
          <aside className="queue-panel" id="schedule">
            <div className="control-section-heading"><div><p className="eyebrow">Today</p><h2>Match queue</h2></div><span>{matchStatus === "completed" ? "2 remaining" : "3 remaining"}</span></div>
            <div className="queue-list">{queue.map((item) => {
              const state = item.id === "UF" && matchStatus === "completed" ? "Completed" : item.state;
              const time = item.id === "UF" && matchStatus === "completed" ? "Final" : item.time;
              return <article key={item.id}><span className="queue-id">{item.id}</span><div><strong>{item.stage}</strong><p>{item.teams}</p></div><div><time>{time}</time><em className={state.toLowerCase()}>{state}</em></div></article>;
            })}</div>
            <Link className="text-action" href="/#schedule">Open public schedule <ChevronRight size={16} /></Link>
          </aside>
        </div>
        ...
        <section className="participants-panel" id="participants">
          ...
          <div className="participant-table" role="table" aria-label="Tournament participants">
            <div className="table-row table-head" role="row"><span role="columnheader">Seed</span><span role="columnheader">Participant</span><span role="columnheader">Region</span><span role="columnheader">Status</span></div>
            {participants.map((item, index) => <div className="table-row" role="row" key={item.id}><span role="cell">{String(index + 1).padStart(2, "0")}</span><span role="cell"><i className="participant-avatar">{item.name.slice(0, 2).toUpperCase()}</i><b>{item.name}</b></span><span role="cell">{item.region}</span><span role="cell"><em className={item.status === "Checked in" ? "ok" : "pending"}>{item.status}</em></span></div>)}
          </div>
        </section>
